$(function(){
	...
	if($(".menu-principal").length){
		$("body").prepend("<div class=\"menu-mobile\"><span>Menu</span>"+$(".logo-mobile").eq(0).parent().html()+$(".menu-principal ul").eq(0).parent().html()+"</div><div class=\"clear\"></div>").find(".menu-mobile ul").hide();
    ...
    $('.mais').mouseenter(function(){
    	if($(".popup-redes-sociais-animado").length==0 && $(".popup-redes-sociais-carregado").length==0){
    		...
    		$.ajax({
    			type:"POST",
    			url:url+"inc-y/redes-sociais.php",
    			data:{ajax:true}
